                      ...
                      const void   *b)
{
    ...
    memcpy_loop (colperm, A->colperm, A->shape[1]);
    ...
    {
        memcpy_loop (A->rowperm, rowperm, A->shape[0]);
    }

    if (options.ColPerm != MY_PERMC)
    {
        memcpy_loop (A->colperm, colperm, A->shape[1]);
    }
    ...
    SUPERLU_FREE (colind);
    SUPERLU_FREE (rowptr);
    SUPERLU_FREE (colperm);
    SUPERLU_FREE (rowperm);
